...
Le scenario fonctionne mieux comme boucle d'enquete tactique : les **Investigators** pensent d'abord devoir survivre a un debarquement, alors que le vrai objectif jouable est de comprendre la structure de commandement Mimic, rejeter la fausse cible, identifier l'emplacement de l'Omega, et preserver un **Now** final coherent apres la rupture de la boucle.
...
L'assaut est un guet-apens. Les Mimics savent que l'attaque arrive parce que leur organisme de commandement, l'**Omega**, utilise un reseau de reset via les Alpha Mimics. Alice tue un Alpha et est aspergee par son sang. A partir de cet instant, sa mort ouvre toujours la meme boucle : elle revient au matin avant le deploiement avec les souvenirs des tentatives precedentes.
...
- Les Mimics ne font pas que predire la coalition. Ils corrigent les issues defavorables via le reseau Omega.
- Alice devient la **Loop Bearer** apres avoir tue un Alpha et recu son sang.
- La boucle ramene toujours Alice au meme point de depart tant que le pouvoir n'est pas perdu.
- Rita Vrataski a eu le meme pouvoir a Verdun et l'a perdu apres une transfusion.
- Le docteur Carter sait que l'ennemi est un organisme distribue : les Drones agissent, les Alphas signalent, l'Omega reset.
...
- Tuer l'Omega neutralise le reseau Mimic et reecrit le **Now** final observable.
- Si la **Main Timeline** finale tue l'Omega mais rend la boucle d'Alice impossible, la realite diverge sauf si un reset par sang d'Alpha ou une ancre equivalente est preserve.
...
| 9 | Alice tue un Alpha et meurt dans son sang. | Cela cree la **Blood Loop**. |
| 10 | Alice reset au matin avant le deploiement. | Elle se souvient. Le reste du monde non. |
...
| 20 | Alice detruit l'Omega et est exposee a nouveau au sang d'Alpha. | Le Now reset vers un etat de victoire avant la rencontre avec Brigham. |
...
flowchart LR
  ...
  TU09 --> TU10["TU10<br/>Reset du matin"]
  ...
  TU19 --> TU20["TU20<br/>Omega detruit, Now reset"]
```
...
Ne revele pas d'abord que la mort d'Alice peut reset la boucle, que la Suisse est fausse, ou que l'Omega se trouve sous le Louvre.
...
| F05 | Dependency | Dependency: F04. Alice tue un Alpha en mourant. | Alice recoit la Blood Loop. | Cadavre d'Alpha, exposition au sang noir, memoire de reset. |
| F06 | Dependency | Dependency: F05. Alice meurt apres le transfert de sang Alpha. | Alice reset au matin du deploiement. | Reveil repete, baraquement inchange, memoire conservee. |
...
| F14 | Dependency | Dependency: F13. Le sang d'Alpha atteint Alice pendant la mort d'Omega. | Le Now reset vers un etat de victoire. | Effondrement Mimic, reset avant la reunion, Rita vivante au baraquement. |
...
flowchart LR
  ...
  F05 --> C06["C06 Dependency<br/>Alice meurt"]:::dependency --> F06["F06<br/>Reset du matin"]:::fact --> E06["E06<br/>Memoire conservee"]:::evidence
  ...
  F13 --> C14["C14 Dependency<br/>Sang d'Alpha pendant la mort d'Omega"]:::dependency --> F14["F14<br/>Now de victoire reset"]:::fact --> E14["E14<br/>Effondrement Mimic"]:::evidence
```

## Regle speciale : Blood Loop

La **Blood Loop** est une regle de scenario, pas un pouvoir par defaut.

- Seule la **Loop Bearer** conserve automatiquement la memoire.
- Avant la transfusion, la mort de la **Loop Bearer** ne termine pas le scenario. Elle la ramene a la **Time Unit** 10.
...
- La transfusion de la **Time Unit** 16 termine la **Blood Loop**. Apres ce point, la mort est finale sauf si une autre regle ou l'evenement Omega final change le **Now**.
- Un reset de boucle ne **Merge** pas automatiquement les facts. Le groupe doit toujours prouver la chaine causale par les Evidence.
...
| Omega | Source de commandement alien | Cible finale cachee sous le Louvre. |
...
- les **Rewind Dice** sont a usage unique sauf si le **MJ** les rafraichit explicitement lors d'un reset de boucle.

Gestion simple recommandee : la **Blood Loop** reset la position de scene et les connaissances, mais pas les **Rewind Dice** depenses. Cela garde l'energie du **System** importante et evite les tentatives mecaniques infinies.
...
| 10 | 10 | d10 | Qu'est-ce qui reset quand Alice meurt ? |
...
- Alice evite la transfusion mais le groupe utilise les resets indefiniment au lieu d'atteindre une convergence finale.
- L'Omega est detruit sans explication coherente pour le reset final.
...
| Victoire tactique, rupture causale | Omega est detruit, mais aucun reset stable ou ancre de boucle n'explique le Now final. | La guerre est gagnee, mais la realite des Investigators diverge de l'origine. |
