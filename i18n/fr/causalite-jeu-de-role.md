# Causalité — Jeu d’enquête causale collaborative

## 1. Concept général

**Causalité** est un jeu de rôle collaboratif d’enquête dans lequel les personnages ne voyagent pas réellement dans le temps.

Le postulat de l’univers est le suivant :

> **Le temps n’existe pas comme une dimension que l’on peut parcourir. Le présent est l’état observable du monde, produit par l’ensemble des causalités passées.**

Les personnages sont des **enquêteurs causaux**. Ils utilisent un dispositif appelé **le Système**, capable d’ouvrir temporairement une **fenêtre de probabilité**. Dans cette fenêtre, plusieurs états possibles peuvent coexister, chacun étant issu d’un enchaînement causal différent.

Les enquêteurs peuvent alors :

- reproduire une partie des causalités passées ;
- revenir à un état antérieur ;
- créer une Branched Timeline causale ;
- agir dans cette Branched Timeline ;
- tenter de la fusionner avec l’état présent.

Le but n’est pas de « changer le passé », mais de comprendre et de reconstruire les causalités qui expliquent le présent, tout en préservant la cohérence de l’état observable.

---

## 2. Vocabulaire fondamental

### État

Un **état** est une configuration observable du monde.

Dans le jeu, chaque unité de la carte causale représente un état.

### Causalité

Une **causalité** est une relation par laquelle un élément, une action ou un fait transforme un état en un autre.

Une causalité peut être représentée comme :

```text
État A → causalité → État B
```

### Passé

Le passé est l’ensemble des causalités qui ont produit l’état présent.

### Présent

Le présent est l’unique état actuellement observable.

Il n’est ni « choisi » ni nécessairement « stabilisé » : il est simplement l’état résultant que les observateurs peuvent percevoir.

### Futur

Le futur correspond à des causalités qui ne se sont pas encore produites.

Il n’est donc pas accessible : on ne peut explorer que des causalités existantes ou reproduire des enchaînements causaux passés.

### Superposition d’états

Lorsque le Système ouvre une fenêtre de probabilité, il maintient plusieurs **états possibles en superposition**.

Ces états sont issus de chaînes causales différentes.

Ce ne sont pas les causalités elles-mêmes qui sont en superposition, mais les états possibles produits par ces causalités.

### État résultant

Lorsque la fenêtre de probabilité se ferme, un seul état demeure observable : l’**état résultant**.

---

## 3. Le Système

Le **Système** est le dispositif qui permet aux enquêteurs d’explorer la causalité.

Il peut :

- ouvrir une fenêtre de probabilité ;
- reproduire une portion des causalités passées ;
- injecter les enquêteurs dans une reproduction d’un état antérieur ;
- maintenir plusieurs Branched Timelines causales ouvertes ;
- afficher la carte causale connue ;
- comparer une Branched Timeline à la Main Timeline ;
- réexécuter les causalités jusqu’au présent lors d’un merge ;
- détecter les conflits ;
- calculer les divergences restantes lors de la résolution finale.

Le Système ne transporte donc pas physiquement les personnages dans le passé.

Il **réexécute les causalités passées à partir d’un état donné**, puis permet aux enquêteurs d’intervenir dans cette nouvelle exécution.

---

## 4. Les bagues des enquêteurs

Chaque enquêteur possède une bague reliée au Système.

Les bagues permettent :

- de consulter la carte causale connue ;
- de connaître la position des autres enquêteurs ;
- de voir les Branched Timelines actives ;
- de communiquer entre enquêteurs ;
- d’identifier les faits validés sur la Main Timeline ;
- de suivre les conflits et les merges ;
- de mesurer l’énergie temporelle disponible ;
- de suivre sa Volonté actuelle.

Les bagues justifient dans l’univers la vision collective et collaborative du plateau de jeu.

---

## 5. Les trois niveaux de réalité

### 5.1 La structure cachée du maître de jeu

Le maître de jeu prépare une structure causale cachée.

Elle contient :

- les faits préparés ;
- leurs conditions ;
- leurs relations causales ;
- les preuves qu’ils produisent ;
- les éléments qui n’ont pas encore été découverts.

Cette structure représente l’enquête et la logique causale préparées par le maître de jeu.

Elle ne doit pas être confondue avec la Main Timeline visible.

### 5.2 La Main Timeline partagée

La **Main Timeline** est l’état partagé et actuellement observable par les joueurs.

Elle est représentée au centre de la table, par exemple sur une ardoise effaçable.

Elle contient toujours **20 Time Units**.

- La **Time Unit 20** représente l’état présent au début de la partie.
- Les **Time Units** précédentes représentent des états antérieurs.
- Chaque **Time Unit** est **Atomic** : les Investigators ne peuvent pas voyager dans une sous-période d’une Time Unit, ni choisir un point entre deux Time Units.
- L’échelle réelle dépend du scénario : vingt Time Units peuvent représenter une journée, dix ans ou plusieurs siècles.

Tout ce qui est inscrit sur la Main Timeline est vrai dans l’état partagé actuel.

Les faits peuvent avoir deux origines :

- un fait révélé et inscrit par le maître de jeu ;
- un fait créé par un joueur puis intégré lors d’un merge réussi.

Un fait inscrit sur la Main Timeline peut être modifié ultérieurement par une Branched Timeline antérieure et un nouveau merge.

### 5.3 Les Branched Timelines

Une Branched Timeline est un état causal alternatif temporairement maintenu par le Système.

Elle constitue une proposition de modification de la Main Timeline.

Les faits d’une Branched Timeline sont réels pour le personnage qui les vit, mais ils ne deviennent pas automatiquement des faits de l’état partagé.

Ils doivent être mergés.

---

## 6. Préparation du maître de jeu

### 6.1 La carte causale

Le maître de jeu prépare une Main Timeline de 20 Time Units.

Il place les faits importants sur les unités concernées.

Il peut également placer les preuves à l’endroit où elles sont produites ou découvertes.

### 6.2 La table causale

Le maître de jeu prépare une table avec les colonnes suivantes :

| Identifiant | Conditions | Fait | Preuves |
|---|---|---|---|
| F01 | La bombe est présente et le détonateur fonctionne | Le laboratoire explose | Article, débris, témoignages |
| F02 | F01 est vrai et l’enquête officielle est ouverte | Une enquête policière commence | Dossier de police |
| F03 | Alice possède le bon code | Alice désamorce la bombe | Bombe intacte, journal d’Alice |

### 6.3 Les conditions

Une condition définit ce qui doit être vrai pour qu’un fait puisse exister.

Il existe deux formes de conditions.

#### Condition simple

Une condition simple décrit un état du monde.

Exemples :

- la porte est ouverte ;
- la bombe est présente ;
- Alice possède le code ;
- le laboratoire est occupé.

#### Condition de dépendance

Une condition de dépendance relie un fait à un autre fait.

Exemples :

- F02 exige que F01 soit vrai ;
- la publication du rapport exige que le chercheur soit vivant ;
- le témoignage exige que le témoin ait assisté à l’événement.

Dans la représentation en graphe :

- les nœuds sont les faits ;
- les arêtes représentent les dépendances entre les faits ;
- les conditions simples peuvent être attachées aux nœuds ou aux relations ;
- chaque fait possède une liste de preuves.

### 6.4 Les faits

Un fait est quelque chose qui se produit dans un état.

Exemples :

- une explosion se produit ;
- Alice ouvre le coffre ;
- un témoin disparaît ;
- un rapport est publié.

Les conséquences d’un fait ne constituent pas nécessairement une catégorie séparée.

Un fait peut devenir une condition pour un fait ultérieur.

### 6.5 Les preuves

Une preuve est une trace observable produite par un fait.

Exemples :

- une photographie ;
- un article de journal ;
- un objet ;
- un dossier ;
- un témoignage ;
- des débris ;
- un enregistrement ;
- une cicatrice ;
- un souvenir.

Les joueurs découvrent la réalité cachée à travers les preuves.

Ils peuvent également produire de nouvelles preuves par leurs actions.

---

## 7. Début de partie

La partie commence à l’unité correspondant au présent, par exemple l’unité 20.

Au départ :

- aucune Branched Timeline n’a encore été créée ;
- la Main Timeline visible est partiellement vide ;
- le maître de jeu remet aux joueurs un dossier initial ;
- ce dossier contient plusieurs preuves ;
- les preuves permettent de révéler ou de placer certains faits sur la Main Timeline.

Le dossier initial peut contenir :

- des photographies ;
- des articles ;
- des objets ;
- des témoignages ;
- des rapports ;
- des enregistrements ;
- des éléments contradictoires.

Le maître de jeu inscrit les faits confirmés sur la Main Timeline.

Il n’inscrit pas les simples hypothèses.

Toutefois, les faits provisoires produits dans une Branched Timeline peuvent être écrits sur la Branched Timeline concernée.

---

## 8. Énergie causale et Rewind Dice

Les **Rewind Dice** représentent l’énergie nécessaire pour maintenir ouverte la fenêtre de probabilité.

Chaque personnage dispose d’un set de dés D&D classique : d4, d6, d8, d10, d10 percentile, d12 et d20.

Les d4, d6, d8, d10, d12 et d20 servent de **Rewind Dice**. Le d10 percentile sert aux tests de **Volonté**. Les d4, d6, d8 et d10 servent aux dégâts.

Un **Rewind Die** peut être dépensé pour ouvrir une Branched Timeline et reproduire une portion plus ou moins longue de la causalité passée.

### 8.1 Portée d’un saut

La taille ou la valeur du **Rewind Die** peut représenter une quantité d’unités causales.

Une proposition de fonctionnement :

- chaque **Rewind Die** fournit un budget d’unités ;
- le coût du rewind dépend de la distance entre le présent et l’unité visée ;
- plus le point causal est éloigné, plus le **Rewind Die** nécessaire est important.

L’échelle exacte reste à équilibrer pendant les tests.

### 8.2 Ressource limitée

Les **Rewind Dice** consommés ne sont pas récupérés pendant la fenêtre de probabilité, sauf règle particulière.

Quand tous les **Rewind Dice** de tous les enquêteurs sont consommés :

- le Système ne peut plus maintenir les superpositions ;
- la fenêtre de probabilité se referme ;
- les états alternatifs cessent d’être observables ;
- la résolution finale commence.

Cette limite constitue le chronomètre stratégique de la partie.

---

## 9. Tour de jeu

À son tour, un joueur peut notamment :

1. agir dans la Branched Timeline où il se trouve ;
2. ouvrir une nouvelle Branched Timeline à partir d’une unité antérieure ;
3. poursuivre une Branched Timeline existante ;
4. tenter de résoudre un conflit ;
5. demander le merge de sa Branched Timeline.

Une Branched Timeline ne limite pas nécessairement le nombre d’actions narratives que le joueur peut y accomplir.

La limite principale est l’énergie consommée pour ouvrir et maintenir l’exploration causale.

Pour éviter les temps morts, la narration peut tourner entre les joueurs même s’ils se trouvent dans différentes Branched Timelines.

---

## 10. Création d’une Branched Timeline

Pour créer une Branched Timeline :

1. le joueur choisit une unité connue ou accessible ;
2. il dépense le dé ou l’énergie nécessaire ;
3. le Système reproduit les causalités jusqu’à cet état ;
4. une Branched Timeline est tracée à partir de l’unité choisie ;
5. le joueur est injecté dans cette exécution causale ;
6. il mène ses actions ;
7. le maître de jeu détermine les faits et les preuves produits.

### Cohérence locale

Pendant qu’une Branched Timeline est jouée, le maître de jeu assure une **cohérence locale**.

Il détermine :

- ce qui existe dans cet état ;
- quelles conditions sont satisfaites ;
- quels faits peuvent se produire ;
- quels effets sont perceptibles ;
- quelles preuves sont produites.

La Branched Timeline peut diverger de la Main Timeline.

Cette divergence n’est pas immédiatement une erreur : elle constitue précisément l’objet de l’exploration.

---

## 11. Antériorité des Branched Timelines

Les Branched Timelines sont ordonnées par les unités dont elles partent.

Une Branched Timeline ouverte à l’unité 14 peut modifier une condition dont dépend une action réalisée dans une Branched Timeline ouverte à l’unité 15.

Exemple :

1. Alice ouvre une Branched Timeline à l’unité 15.
2. Elle récupère dans un coffre un code permettant de désamorcer une bombe.
3. Un autre enquêteur ouvre ensuite une Branched Timeline à l’unité 14.
4. Il remplace le papier dans le coffre par un faux code.
5. Lorsque la causalité est réexécutée, Alice récupère désormais le faux code.
6. La bombe peut exploser conformément au fait présent sur la Main Timeline.

Brancher plus tôt permet donc de modifier les conditions d’un fait ou d’une Branched Timeline ultérieure.

---

## 12. Le merge

Le merge est la tentative d’intégrer les faits d’une Branched Timeline à la Main Timeline.

Narrativement, le Système :

1. prend les modifications produites dans la Branched Timeline ;
2. réexécute les causalités depuis le point de branchement ;
3. recalcule les états successifs ;
4. compare l’état résultant à la Main Timeline actuelle ;
5. détecte les incompatibilités ;
6. intègre les changements compatibles.

Le merge revient toujours vers le présent, c’est-à-dire vers la dernière unité de la Main Timeline.

Il ne s’agit pas d’un voyage vers le futur : le Système réexécute les causalités jusqu’à l’état présent.

---

## 13. Analyse d’un merge

Lors d’un merge, le maître de jeu ne compare pas chaque action narrative.

Il compare principalement :

- les faits créés ;
- les faits supprimés ;
- les faits modifiés ;
- les conditions modifiées ;
- les dépendances causales invalidées ;
- les preuves qui doivent apparaître ou disparaître.

### Propagation causale

Si une condition d’un fait n’est plus remplie :

1. le fait devient incompatible ;
2. ses preuves peuvent disparaître ;
3. les faits qui dépendent de lui doivent être vérifiés ;
4. le conflit peut se propager dans la chaîne causale.

---

## 14. Conflits majeurs

Un conflit est **majeur** lorsqu’une modification de Branched Timeline contredit un fait ou une structure causale préparés dans la table du maître de jeu, ou lorsqu’elle rend impossible un élément essentiel de l’enquête.

Un conflit majeur :

- bloque le merge ;
- ne peut pas être écrasé par un simple choix ;
- ne peut pas être résolu par un jet de dés ;
- exige une action causale supplémentaire ;
- doit être traité dans la Branched Timeline actuelle ou dans une Branched Timeline corrective dédiée.

### Exemple

La Main Timeline contient le fait :

> Le laboratoire explose à l’unité 16.

Alice désamorce la bombe dans sa Branched Timeline.

La Branched Timeline ne peut pas merger tant qu’une autre cause compatible de l’explosion n’a pas été créée.

Un enquêteur peut ouvrir une Branched Timeline plus tôt pour :

- remplacer le code d’Alice ;
- ajouter une seconde bombe ;
- empêcher Alice d’accéder au coffre ;
- provoquer une explosion d’une autre manière.

Le conflit majeur disparaît lorsque le fait établi redevient causalement possible.

---

## 15. Conflits mineurs

Un conflit est **mineur** lorsqu’il oppose deux faits compatibles avec la structure essentielle du scénario, notamment :

- deux Branched Timelines de joueurs qui modifient la même unité ;
- deux versions possibles d’un événement non structurant ;
- une modification locale qui ne remet pas en cause la table causale fondamentale du maître de jeu.

Dans un conflit mineur, le joueur choisit la version qu’il souhaite imposer :

- conserver la version de sa Branched Timeline ;
- conserver la version actuelle de la Main Timeline.

Il effectue ensuite un jet de Volonté.

### Résolution

- Si le jet réussit, le choix du joueur est appliqué.
- Si le jet échoue, la décision inverse est appliquée.

Cela reproduit la logique d’un conflit de merge : le joueur tente d’imposer soit sa modification, soit l’état actuel.

### Chevauchement de Branched Timelines

Si deux Branched Timelines touchent la même unité ou modifient le même fait, leur incompatibilité est généralement traitée comme un conflit mineur, sauf si elle affecte un fait structurel de l’enquête.

---

## 16. Volonté

Chaque personnage possède :

- une **Volonté maximale** ;
- une **Volonté actuelle**.

La Volonté représente sa capacité à maintenir la cohérence entre ce qu’il a vécu dans les Branched Timelines et l’état actuellement observable.

Par défaut, un Investigator humain commence avec une **Volonté maximale de 100**.

### 16.1 Calcul à chaque tour

À la fin du tour de chaque joueur, la Volonté actuelle de ce joueur est recalculée :

```text
Volonté actuelle
= 100
− modificateur du tour
```

Le modificateur du tour est calculé ainsi :

```text
modificateur du tour
= 10 x nombre de ses Branched Timelines non Merged
+ 10 x nombre de ses conflits majeurs non résolus
+ 5 x nombre de ses conflits mineurs non résolus
+ autres pénalités actives de Volonté
```

Seules les Branched Timelines non Merged et les conflits non résolus du personnage concerné augmentent directement ce modificateur, sauf règle ou conséquence contraire.

Le joueur doit toujours conserver une Volonté actuelle strictement supérieure à 0 à la fin de son tour. Si le calcul réduit la Volonté actuelle à 0 ou moins, le personnage sombre dans la folie et ne peut plus maintenir une relation cohérente avec le Now observable.

### Exemple

Alice possède une Volonté maximale de 100.

Elle a :

- 2 Branched Timelines non Merged ;
- 1 conflit majeur non résolu ;
- 1 conflit mineur non résolu.

Sa Volonté actuelle est donc :

```text
modificateur du tour = (2 x 10) + (1 x 10) + (1 x 5) = 35
Volonté actuelle = 100 - 35 = 65
```

### 16.2 Jet de Volonté

Pour tenter de résoudre un conflit mineur, le joueur lance un seul **d10 percentile**.

Le dé est lu en dizaines :

```text
00, 10, 20, 30, 40, 50, 60, 70, 80, 90
```

La face `00` vaut `0`, pas `100`.

Le seuil du test est :

```text
seuil = 100 - Volonté effective
```

- Si le résultat du d10 percentile est **supérieur ou égal** au seuil, le jet réussit.
- Si le résultat est **inférieur** au seuil, le jet échoue.

Exemple avec une Volonté actuelle de 97 en difficulté moyenne :

```text
seuil = 100 - 97 = 3
```

- 00 : échec ;
- 10, 20, 30, 40, 50, 60, 70, 80 ou 90 : réussite.

### Difficulté

La difficulté modifie la Volonté effective avant le calcul du seuil. La valeur finale est tronquée.

| Difficulté | Volonté effective |
|---|---|
| Très facile | Volonté actuelle x 10 |
| Facile | Volonté actuelle x 2 |
| Moyenne | Volonté actuelle |
| Difficile | Volonté actuelle / 2 |
| Très difficile | Volonté actuelle / 4 |
| Impossible | Volonté actuelle / 100 |

### 16.3 Volonté nulle

Si la Volonté atteint zéro ou moins, le personnage sombre dans la folie. Il ne peut normalement plus effectuer de jet de Volonté tant que la table n’a pas résolu cet état.

Cela peut représenter :

- une incapacité à distinguer l’état observable des états divergents ;
- la folie ;
- une impossibilité d’imposer sa version lors d’un merge.

Le traitement exact de la Volonté nulle reste à préciser.

---

## 17. Branched Timelines non Merged et divergence psychique

Aucune Branched Timeline n’est simplement effacée ou oubliée.

Toutes les Branched Timelines restent dessinées et visibles jusqu’à la fin de la partie.

Une Branched Timeline peut être :

- ouverte ;
- en attente de merge ;
- mergée ;
- non Merged ;
- bloquée par un conflit.

Une Branched Timeline non mergée reste une réalité vécue par le personnage, mais elle ne fait pas partie de l’état partagé.

Le personnage conserve donc des souvenirs de faits qui ne correspondent pas à l’état observable.

Cette divergence est la source de la dégradation psychique.

---

## 18. Résolution finale

Lorsque toute l’énergie causale représentée par les dés est consommée :

1. le Système ne peut plus maintenir la fenêtre de probabilité ;
2. la superposition des états possibles cesse d’être perceptible ;
3. les Branched Timelines alternatives se referment ;
4. l’état résultant devient l’unique état observable ;
5. les divergences de chaque personnage sont calculées ;
6. les conséquences psychiques sont appliquées ;
7. l’enquête est évaluée.

Les faits vécus dans des Branched Timelines non mergées restent dans la mémoire des enquêteurs.

Le nombre ou le poids de ces faits divergents peut produire des pénalités finales de Volonté ou déterminer si le personnage sombre dans la folie.

Le barème final exact reste à définir.

---

## 19. Conditions de victoire et fins possibles

### Convergence complète

- L’enquête est résolue.
- Les causalités nécessaires au présent sont cohérentes.
- L’état résultant correspond au présent recherché.
- Tous les enquêteurs conservent une continuité psychique suffisante.
- Les divergences restantes sont nulles ou supportables.

Il s’agit de la meilleure fin.

### Convergence incomplète

- Un état cohérent demeure observable.
- Une partie de l’enquête reste irrésolue.
- Certaines Branched Timelines ou certains conflits n’ont pas été compris.

### Divergence psychique

- L’état résultant est cohérent.
- Un ou plusieurs enquêteurs conservent trop de souvenirs d’états devenus inobservables.
- Leur Volonté s’effondre ou ils sombrent dans la folie.

### Rupture causale

- Les joueurs ne parviennent pas à produire un état résultant cohérent.
- Des conflits majeurs demeurent au moment de la fermeture.
- Le présent ne peut plus être expliqué par les causalités restantes.
- Les enquêteurs ou certains éléments de l’état initial peuvent disparaître.

---

## 20. Plateau de jeu

### Main Timeline centrale

La Main Timeline partagée est dessinée au centre de la table.

Elle peut prendre la forme :

```text
01 — 02 — 03 — 04 — ... — 18 — 19 — 20
                                      PRÉSENT
```

Le maître de jeu y inscrit les faits devenus vrais et observables.

### Branched Timelines

Les joueurs tracent leurs Branched Timelines au-dessus ou au-dessous de la Main Timeline.

```text
MAIN :  01 — 02 — 03 — 04 — 05 — 06 — ... — 20
                       \
BRANCHE A :             A1 — A2 — A3 — MERGE
                  \
BRANCHE B :        B1 — B2 — MERGE
```

Aucune Branched Timeline n’est effacée.

Elles constituent :

- la mémoire de la partie ;
- l’historique des tentatives ;
- la trace des réalités vécues ;
- la base de calcul des divergences.

### Informations affichées

Chaque Branched Timeline devrait indiquer :

- son propriétaire ;
- son unité de départ ;
- les faits créés ou modifiés ;
- ses conflits ;
- son statut ;
- son coût en énergie ;
- son point de merge éventuel.

---

## 21. Rôle du maître de jeu

Le maître de jeu :

- prépare la table causale ;
- prépare le graphe de faits ;
- place les preuves initiales ;
- maintient les informations cachées ;
- révèle les faits découverts ;
- inscrit les faits validés sur la Main Timeline ;
- évalue la cohérence locale des Branched Timelines ;
- transforme les actions en faits ;
- détermine les preuves produites ;
- analyse les merges ;
- identifie les conflits majeurs et mineurs ;
- propage les modifications causales ;
- maintient la lisibilité du plateau.

Le maître de jeu ne doit pas tout improviser ni tout simuler.

Il se concentre sur les faits structurants et leurs conditions.

---

## 22. Rôle des joueurs

Les joueurs :

- étudient les preuves ;
- reconstruisent les faits ;
- choisissent les points de branchement ;
- dépensent leur énergie causale ;
- mènent des actions dans les Branched Timelines ;
- créent ou modifient des faits ;
- coopèrent pour résoudre les conflits ;
- tentent des merges ;
- ouvrent des Branched Timelines correctives ;
- surveillent leur Volonté ;
- cherchent à résoudre l’enquête avant la fermeture de la fenêtre.

Le système est coopératif.

Les joueurs voient :

- la Main Timeline partagée ;
- les Branched Timelines actives ;
- les Branched Timelines des autres joueurs ;
- les faits connus ;
- les conflits visibles ;
- les ressources restantes.

Ils ne voient pas :

- les faits cachés non découverts ;
- les conditions secrètes ;
- les preuves encore inconnues ;
- l’intégralité du graphe causal du maître de jeu.

---

## 23. Boucle principale

```text
1. Observer les preuves et la Main Timeline.
2. Formuler un objectif d’enquête.
3. Choisir une unité causale.
4. Dépenser un Rewind Die pour ouvrir une Branched Timeline.
5. Agir dans la Branched Timeline.
6. Produire ou modifier des faits.
7. Évaluer la cohérence locale.
8. Tenter un merge.
9. Détecter les conflits.
10. Résoudre les conflits mineurs par choix et jet de Volonté.
11. Résoudre les conflits majeurs par des actions ou des Branched Timelines correctives.
12. Mettre à jour la Main Timeline.
13. Recalculer la Volonté du joueur actif à la fin de son tour.
14. Continuer jusqu’à résolution ou épuisement des Rewind Dice.
15. Fermer la fenêtre et déterminer l’état résultant.
```

---

## 24. Résumé des mécaniques

| Élément | Fonction |
|---|---|
| Time Unit | Une des 20 Atomic Time Units de la carte causale |
| Atomic | Propriété indiquant qu'une Time Unit ne peut pas être divisée ou traversée par une sous-période |
| Main Timeline | État partagé et actuellement observable |
| Structure du MJ | Graphe causal et faits cachés de l’enquête |
| Condition simple | État du monde nécessaire à un fait |
| Condition de dépendance | Lien causal exigeant un autre fait |
| Fait | Événement ou réalité produite dans un état |
| Preuve | Trace observable produite par un fait |
| Branched Timeline | Exploration d’un état alternatif |
| Rewind Dice | Dés d'énergie utilisés pour ouvrir des Branched Timelines : d4, d6, d8, d10, d12 et d20 |
| Merge | Réexécution des causalités jusqu’au présent avec intégration des modifications |
| Conflit majeur | Incompatibilité structurelle exigeant une Branched Timeline corrective |
| Conflit mineur | Opposition locale entre deux versions, résolue par choix et Volonté |
| Volonté | Capacité individuelle à imposer une cohérence |
| Branched Timeline non Merged | Réalité vécue mais non intégrée, réduisant la Volonté |
| Fermeture | Fin de l’énergie et disparition des états alternatifs perceptibles |
| État résultant | Unique état observable après la fermeture |

---

## 25. Formules

### Volonté actuelle

```text
Volonté actuelle du personnage
= 100
− modificateur du tour
```

```text
modificateur du tour
= 10 x Branched Timelines non Merged du personnage
+ 10 x conflits majeurs non résolus du personnage
+ 5 x conflits mineurs non résolus du personnage
+ autres pénalités actives de Volonté
```

```text
La Volonté actuelle doit être > 0 à la fin du tour du joueur
Volonté actuelle <= 0 : le personnage sombre dans la folie
```

### Jet de résolution d’un conflit mineur

```text
Volonté effective = tronquer(Volonté actuelle x modificateur de difficulté)
seuil = 100 - Volonté effective
résultat du d10 percentile >= seuil : réussite
résultat du d10 percentile < seuil : échec
```

### Résolution d’un conflit mineur

```text
1. Le joueur choisit la version qu’il souhaite imposer.
2. Il effectue un jet de Volonté.
3. En cas de réussite, son choix est appliqué.
4. En cas d’échec, la version opposée est appliquée.
```

---

## 26. Exemple synthétique

La Main Timeline indique :

```text
Unité 16 : le laboratoire explose.
Unité 20 : les enquêteurs commencent leur mission.
```

Une preuve révèle qu’Alice avait accès à un code de désamorçage.

### Branched Timeline d’Alice

Alice ouvre une Branched Timeline à l’unité 15.

Elle :

1. ouvre un coffre ;
2. récupère le code ;
3. désamorce la bombe.

Sa Branched Timeline produit le fait :

```text
Fait A : la bombe est désamorcée.
```

Mais ce fait rend impossible :

```text
Fait principal : le laboratoire explose.
```

Le merge est bloqué par un conflit majeur.

### Branched Timeline corrective

Un autre enquêteur ouvre une Branched Timeline à l’unité 14.

Il :

1. entre dans la pièce du coffre ;
2. remplace le code par un faux ;
3. laisse le faux document à sa place.

Lors de la réexécution :

1. Alice récupère le faux code ;
2. elle échoue à désamorcer la bombe ;
3. l’explosion se produit ;
4. le fait de l’unité 16 redevient possible.

La Branched Timeline corrective permet alors de résoudre le conflit majeur.

---

## 27. Questions de conception encore ouvertes

Les éléments suivants restent à définir ou à tester :

1. Comment pondérer les faits divergents lors de la résolution finale ?
2. Tous les faits divergents comptent-ils autant ?
3. Une règle de scénario doit-elle permettre de prêter ou transférer des Rewind Dice ?
4. Une Branched Timeline consomme-t-elle seulement un Rewind Die à l’ouverture ou aussi pendant son maintien ?
5. Comment limiter le temps narratif passé dans une seule Branched Timeline ?
6. Comment traiter plusieurs personnages présents dans la même Branched Timeline ?
7. Un joueur peut-il rejoindre la Branched Timeline d’un autre ?
8. Que se passe-t-il si un conflit mineur devient structurel par propagation ?
9. Les preuves déjà observées peuvent-elles disparaître de la Main Timeline ?
10. Comment les souvenirs des joueurs sont-ils affectés après un merge ?
11. Quelles sont les conséquences précises de chaque niveau de divergence psychique ?
12. Les résultats proposés pour les Rewind Dice sont-ils assez équilibrés en jeu répété ?
13. Les dés de dégâts proposés sont-ils assez équilibrés en jeu répété ?

---

## 28. Manifeste du jeu

> **Le temps n’existe pas.**
>
> **Le présent est l’état observable produit par les causalités passées.**
>
> **Le futur n’est pas accessible, car ses causalités n’existent pas encore.**
>
> **Le Système ne permet pas de remonter le temps : il réexécute la causalité.**
>
> **Une Branched Timeline explore un autre enchaînement causal et produit un autre état possible.**
>
> **Le merge réexécute les causalités jusqu’au présent.**
>
> **Lorsque l’énergie est épuisée, la superposition cesse d’être perceptible.**
>
> **Un seul état résultant demeure observable.**
>
> **Les enquêteurs doivent découvrir la vérité avant que la fenêtre ne se referme.**
