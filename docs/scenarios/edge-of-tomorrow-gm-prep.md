...
The assault is an ambush. The Mimics know the attack is coming because their command organism, the **Omega**, uses a reset network through Alpha Mimics. Alice kills an Alpha and is soaked in its blood. From that moment, her death opens the same loop again: she returns to the morning before deployment with memories of previous attempts.
...
- The Mimics are not merely predicting the coalition. They are correcting failed outcomes through the Omega network.
- Alice becomes the Loop Bearer after killing an Alpha and receiving its blood.
- The loop always returns Alice to the same starting point until the power is lost.
- Rita Vrataski had the same power at Verdun and lost it after a transfusion.
- Doctor Carter knows the enemy is a distributed organism: Drones act, Alphas signal, Omega resets.
...
- Killing the Omega neutralizes the Mimic network and rewrites the final observable Now.
- If the final Main Timeline kills the Omega but makes Alice's loop impossible, reality diverges unless an Alpha-blood reset or equivalent loop anchor is preserved.
...
| 9 | Alice kills an Alpha and dies in its blood. | This creates the Blood Loop. |
| 10 | Alice resets to the morning before deployment. | She remembers. The rest of the world does not. |
...
| 20 | Alice destroys the Omega and is exposed to Alpha blood again. | The Now resets to a victory state before Brigham's meeting. |
...
flowchart LR
  ...
  TU09 --> TU10["TU10<br/>Reset morning"]
  ...
  TU19 --> TU20["TU20<br/>Omega destroyed, Now reset"]
```
...
Do not reveal at first that Alice's death can reset the loop, that Switzerland is false, or that the Omega is beneath the Louvre.
...
| F05 | Dependency | Dependency: F04. Alice kills an Alpha while dying. | Alice receives the Blood Loop. | Alpha corpse, black blood exposure, reset memory. |
| F06 | Dependency | Dependency: F05. Alice dies after the Alpha blood transfer. | Alice resets to the deployment morning. | Repeated wake-up scene, unchanged barracks, retained memory. |
...
| F14 | Dependency | Dependency: F13. Alpha blood reaches Alice during the Omega death event. | The Now resets to a victory state. | Mimic collapse, pre-meeting reset, Rita alive at barracks. |
...
flowchart LR
  ...
  F05 --> C06["C06 Dependency<br/>Alice dies"]:::dependency --> F06["F06<br/>Reset morning"]:::fact --> E06["E06<br/>Retained memory"]:::evidence
  ...
  F13 --> C14["C14 Dependency<br/>Alpha blood in Omega death"]:::dependency --> F14["F14<br/>Victory Now reset"]:::fact --> E14["E14<br/>Mimic collapse"]:::evidence
```

## Special Rule: Blood Loop

The Blood Loop is a scenario rule, not a default power.

- Only the Loop Bearer has automatic memory continuity.
- Before transfusion, the Loop Bearer's death does not end the scenario. It returns the Loop Bearer to Time Unit 10.
...
- The transfusion at Time Unit 16 ends the Blood Loop. After that point, death is final unless another rule or final Omega event changes the Now.
- A loop reset does not automatically **Merge** facts. The group must still prove the causal chain through Evidence.
...
| Omega | Alien command source | Hidden final target beneath the Louvre. |
...
- Rewind Dice are single-use unless the GM explicitly refreshes them at a loop reset.

Recommended simple handling: the Blood Loop resets scene position and knowledge, but not spent Rewind Dice. This keeps System energy meaningful and prevents infinite mechanical attempts.
...
| 10 | 10 | d10 | What resets when Alice dies? |
...
- Alice avoids the transfusion but the group uses loop resets forever instead of reaching final convergence.
- Omega is destroyed without a coherent explanation for the final reset.
...
| Tactical victory, causal rupture | Omega is destroyed, but no stable reset or loop anchor explains the final Now. | The war is won, but the Investigators' reality diverges from origin. |
